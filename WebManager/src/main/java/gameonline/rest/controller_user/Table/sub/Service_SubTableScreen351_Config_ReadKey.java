...
public class Service_SubTableScreen351_Config_ReadKey extends BaseAuthorization{
		...
		new ClientOneHit(regionId) {
			...
			@Override public void onReceiveMessage(MessageReceiving messageReceiving) {
				...
				if(caseCheck == CaseCheck.HOPLE) {
					long readToken = messageReceiving.readLong();
//					System.out.println("accessKey : "+accessKey);
					
					ObjectNode node = new ObjectMapper().createObjectNode();
//					node.put(SystemConstant.AccessToken, binaryToken.longToToken(accessKey));
					node.put(SystemConstant.AccessToken, readToken+"");
			...
			@Override public MessageSending doSendMessage() {
				MessageSending messageSending = new MessageSending(CMD_ONEHIT.BBWeb_SubTable_Config_ReadKey);
