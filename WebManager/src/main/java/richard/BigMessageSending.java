...
public class BigMessageSending {
	public byte[] data;
	public int dataLength;
	public int currentWriter;

	public BigMessageSending() {
		currentWriter=0;
		dataLength=0;
		data = null;
	}
...
//	public final void seekEnd() {currentWriter=dataLength;}
	...
	public final void skip(int numberByte) {
		if (currentWriter + numberByte > dataLength) {
			dataLength = currentWriter + numberByte;
			...
			currentWriter += numberByte;
	}

	public final void initBuffer(int length) {
		if (currentWriter + length > dataLength) {
			dataLength = currentWriter + length;
			byte[] temp = new byte[dataLength];
			for (int i = 0; i < currentWriter; i++)
				temp[i] = data[i];
			data = temp;
	...
	public final void writeByteArrayWithLength(byte[] arr) {
		if (arr == null || arr.length == 0)
			...
			if (currentWriter + lengthArr + 4 > dataLength) {
				dataLength = currentWriter + lengthArr + 4;
				byte[] temp = new byte[dataLength];
				for (int i = 0; i < currentWriter; i++)
					temp[i] = data[i];
				int value = lengthArr;
	...
	public final void writeCopyData(byte[] copyData) {
		...
		if (currentWriter + lengthCopy > dataLength) {
			dataLength = currentWriter + lengthCopy;
			byte[] temp = new byte[dataLength];
			for (int i = 0; i < currentWriter; i++)
				temp[i] = data[i];
...
//	    paramDataOutput.write(arrayOfString, 0, j + 2);
		...
		if (currentWriter + lengString <= dataLength) {
			...
			currentWriter = currentWriter + lengString;
		} else {
	...
	public MessageSending toMessage() {
		MessageSending messageSending=new MessageSending((short)0, data);
		messageSending.currentWriter = (short) currentWriter;
		messageSending.dataLength = (short) dataLength;
