...
public class DBString{
	...
	private int compare;
	private long l;
	public DBString(short tableId) throws FileNotFoundException {
		rfData = new RandomAccessFile(PATH.DATABASE_FOLDER+"/"+tableId+"/"+PATH.StringData, "rw");
		rfBTree = new RandomAccessFile(PATH.DATABASE_FOLDER+"/"+tableId+"/"+PATH.StringIndex, "rw");
	}
	
	public String getStringByLocation(long location) throws IOException {
	...
	public long getStringId(String str) throws IOException {if(str==null)return -1;return getStringId(str.getBytes());}
	private long getStringId(byte[] _data) throws IOException {
		if(rfBTree.length()==0) {
	...
	public long findStringId(byte[] _data) throws IOException {
		data = _data;
	...
	protected void calculateCompare(long id) throws IOException {
		rfBTree.seek(id*8);
